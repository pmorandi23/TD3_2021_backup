...
IRQ01_Handler:
    ...
    pushad
                                              ; Guardo todos los registros  para asegurarme que no se rompa el estado actual.
    ...
    ; ->Comparo teclas para generar FAULTS
    cmp     eax, TECLA_A
    je      DF_generate
    ...
    jmp determinar_numero

DF_generate:    ; Genero #DF
    ; Elimino handler de la división por cero (#DE)
    mov     edi, IDT   
    mov     ecx, 0x02       
    xor     eax, eax           
    rep     stosd   
    ...
    jmp     end_handler_teclado


UD_generate:  ; Genero #UD
...
AC_generate:
    ...
    ; Seteo el bit de Align Check en CR0
    xor     eax, eax
    mov     eax,cr0
    or      eax,X86_CR0_AM 
    mov     cr0,eax

    ; Guardo CR3 de la tarea actual
    mov ebx, cr3

    ; Cargo CR3 de la tarea 2 donde quiero generar la #AC
    mov eax, __CR3_TAREA_2_PHY
    mov cr3, eax

    ; Le asigno al EIP contexto de ejecuacion de la tarea 1 una operación
    ; que genera la falla con nivel de privilegio 3
    xor     eax, eax
    mov     eax, __TSS2_VMA
    mov     [ eax + 36 ], dword(EFLAGS_AC)                    ; EFLAGS 
    mov     [ eax + 32 ], dword(tarea_2_AC_generate)      ; EIP     
    
    ; Le vuelvo a cargar el CR3 de la tarea actual
    mov cr3, ebx

    jmp end_handler_teclado   


SS_generate:   ;#Stack Segment Fault

    xchg bx, bx
    ;lgdt[gdtr_SS]
    mov ax,DS_SEL_SS          ; Privilegio nivel 0, no presente
    mov ss,ax
    jmp end_handler_teclado
...
ISR06_Handler_UD:
    ...
    mov     dl,0x06
    hlt
; Device not avaibable (No Math Coprocesor)
ISR07_Handler_NM:
    mov     dl,0x07
...
ISR128_Handler_SC:
    ...
    cmp eax, SYS_PRINT_VGA
    je sys_print_VGA

    cmp eax, SYS_HALT
    je sys_hlt

; Lectura de palabras de 64 bits (8 bytes)
sys_read:

    push    ebp
    mov     ebp, esp 
    push    ecx         ; Tamaño de tabla
    push    edi          ; Tabla destino
    push    esi          ; Tabla fuente
    call    __fast_memcpy
    leave
