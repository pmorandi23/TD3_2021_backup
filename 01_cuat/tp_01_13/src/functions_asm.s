...
leer_contexto_siguiente_asm:
    ...
    add esp,4
     ...
     ;Seteo el flag CR0.TS = 1 para las tareas 2 y 3 que usan SIMD
    smsw ax
    or ax, 0x0008
    lmsw ax
...
no_guardo_stack_user:
    ; Stack de nivel 0
    mov byte [primer_context_save], 0
    mov [TSS_aux + 8],    ss       ; Guardo SS0
    mov [TSS_aux + 4],    esp      ; Guardo ESP0

    ; Analizo si tengo que guardar contextos MMX
    cmp dword[TSS_aux], __TSS2_VMA
    je guardar_mmx2

    cmp dword[TSS_aux], __TSS3_VMA
    je guardar_mmx3
